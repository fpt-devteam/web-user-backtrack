...
function VerifyEmailIllustration() {
  return (
    <div className="relative w-64 h-64 mx-auto">
      {/* Rose rounded square background */}
      <div className="absolute inset-0 bg-brand-400/70 rounded-[3.5rem]" />
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        ...
        >
          {/* Envelope body */}
          <rect x="5" y="10" width="110" height="70" rx="8" fill="#fde8ec" />
          {/* Envelope flap fold lines */}
          <path d="M5 18 L60 52 L115 18" fill="none" stroke="#fecdd3" strokeWidth="2" />
          {/* Heart */}
          <path
          ...
          />
          {/* Wing left */}
          <path d="M5 18 L30 42 L5 62" fill="#fecdd3" />
          {/* Wing right */}
          <path d="M115 18 L90 42 L115 62" fill="#fecdd3" />
        </svg>
      </div>

      {/* Brand heart badge — top right */}
      <div className="absolute -top-3 -right-3 w-14 h-14 bg-brand-500 rounded-2xl flex items-center justify-center shadow-md">
  ...
  return (
    <div className="flex flex-col flex-1 lg:flex-none lg:items-center">
      ...
      <div className="text-center px-2 mb-10 lg:max-w-sm lg:mx-auto">
        <h1 className="text-3xl font-extrabold text-foreground mb-4">Verify your email</h1>
        <p className="text-muted-foreground text-base leading-relaxed">
          We've sent a verification link to{' '}
          <span className="font-bold text-foreground">{email}</span>. Please click the link to
          secure your account.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3 lg:max-w-sm lg:mx-auto lg:w-full">
        ...
        <button
          type="button"
          onClick={onResend}
          className="w-full py-3 text-base font-bold text-foreground hover:text-muted-foreground transition-colors"
        >
          I didn't get an email
        </button>

        <div className="flex items-center justify-center gap-1.5 pt-1">
          <span className="text-sm text-muted-foreground">Already verified?</span>
          <button
            type="button"
            onClick={onSignIn}
